...
import { Avatar, Incubator, View, RadioGroup, RadioButton, Text, Checkbox, Colors, Button, Icon } from 'react-native-ui-lib';
...
const Register = () => {
    ...
    useEffect(() => {
        if (validName && validGender && validPhone && validPass && isCheckpass && confirm) {
            setValidForm(true);
        } else {
            setValidForm(false);
        }
    ...
    function RenderRadioButton(value, text, select, props) {
        return (
            <View row marginR-10>
                <RadioButton value={value} label={text} selected={select} {...props} />
            </View>
        );
    };
        ...
        <View centerV paddingH-20 paddingT-50>
            ...
            <View>
                <TextField
                    ...
                    onChangeValidity={(isValid) => { setValidName(isValid); console.log("valid name:", isValid); }}
                    maxLength={30}
                />
            </View>
            <View row>
                <RadioGroup row initialValue={this.gender} onValueChange={value => setGender(value)}>
            ...
            <View>
                <TextField fieldStyle={{ borderBottomWidth: 1, padding: 2 }}
                    ...
                    validateOnBlur
                    validationMessage={['Trường bắt buộc', 'Điện thoại chỉ gồm số', 'Số điện thoại không hợp lệ']}
                    onChangeValidity={(isValid) => { setValidPhone(isValid); console.log("phone", isValid); }}
                    maxLength={30}
            ...
            <View>
                <TextField fieldStyle={{ borderBottomWidth: 1, padding: 2 }}
                    ...
                    onChangeValidity={(isValid) => { setValidPass(isValid); console.log("pass", isValid); }}
                    maxLength={30}
                    secureTextEntry={true}
                >
                </TextField>
                {/* <Icon source={{uri: 'https://github.com/wix/react-native-ui-lib/blob/master/demo/src/assets/icons/delete.png'}} size={10} tintColor={Colors.grey40}/> */}
            </View>
            ...
            <View centerH row marginT-20>
                ...
                <Button marginL-10 text80 link label="Đăng nhập" onPress={() => navigation.navigate('LogIn')}></Button>
            </View>
        </View>
    );
}
