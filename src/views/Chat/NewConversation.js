...
const NewConversation = ({ navigation, route }) => {
    ...
    const fetchFriendList = async () => {
        try {
            const { status, data: { data } } = await FriendService.getFriends();
    ...
    const onPressNewConversation = async (friend) => {
        try {
            const { status, data: { data } } = await ChatService.sendMessage({ receivedId: friend._id, type: ChatType.PRIVATE_CHAT });
            if (status == httpStatus.OK)
                navigation.navigate('Message', { chatId: data._id, receivedUser: friend, socket });
        } catch (error) {
            console.log(error);
        }
    }
            ...
            <View style={styles.body}>
                <ScrollView style={styles.scrollView}>
                    ...
                    {friendList.map(friend => (
                        <TouchableOpacity key={friend._id} style={styles.friend} onPress={() => onPressNewConversation(friend)}>
